"""
10_vignetting_cifar10.py
========================================
3x3 Position-Dependent Kernel Vignetting correction — CIFAR-10
...
Run:
  python 10_vignetting_cifar10.py
  python 10_vignetting_cifar10.py --n-images 4 --alpha 2.5
  python 10_vignetting_cifar10.py --data-dir /path/to/CIFAR-10
"""
import argparse
import os
import glob
...
# ============================================================================
# Dataset: CIFAR-10
# torchvision으로 자동 다운로드. grayscale 변환 후 사용.
# 32x32 소형이라 vignetting 효과 확인용.
# ============================================================================
...
def load_dataset(args):
    try:
        ...
        tf = T.Compose([T.Grayscale(), T.ToTensor()])
        ds = torchvision.datasets.CIFAR10(
            root=args.data_dir, train=False, download=True, transform=tf)
        imgs = []
        indices = list(range(0, len(ds), max(1, len(ds)//args.n_images)))
        for i in indices[:args.n_images]:
            img, _ = ds[i]                        # (1, H, W)
            imgs.append(img.unsqueeze(0))         # (1,1,H,W)
        print(f'[Load] CIFAR-10: {len(imgs)} images ({args.img_size}x{args.img_size})')
        return imgs
    except Exception as e:
        print(f'[Warning] CIFAR-10 load failed: {e}')
        print('[Fallback] using synthetic checker pattern')
        ...
        for _ in range(args.n_images):
            ...
            gy, gx = torch.meshgrid(ys, xs, indexing='ij')
            v = (0.5+0.5*torch.sign(torch.sin(gy*8)*torch.sin(gx*8))).clamp(0,1)
            imgs.append(v.unsqueeze(0).unsqueeze(0))
        return imgs


def parse_args():
    p = argparse.ArgumentParser(description='Vignetting PDK — CIFAR-10')
    p.add_argument('--data-dir',  type=str,   default='./data')
    p.add_argument('--n-images',  type=int,   default=4)
    p.add_argument('--img-size',  type=int,   default=32,
                   help='CIFAR-10 native size is 32')
    p.add_argument('--alpha',     type=float, default=2.5)
    p.add_argument('--n-iter',    type=int,   default=300)
    p.add_argument('--lr',        type=float, default=0.05)
    p.add_argument('--res-dir',   type=str,   default='./res/vignetting_cifar10')
    return p.parse_args()
...
def main():
    ...
    save_comparison(raw_list, deg_list, cor_ana_list, cor_learn_list,
                    true_alpha, est_alpha, learn_alpha, args.res_dir, 'CIFAR-10')
