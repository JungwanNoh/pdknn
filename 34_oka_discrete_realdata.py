"""
34_oka_discrete_realdata.py
======================================================================
OKA simulation with DISCRETE voltage levels and MEASUREMENT-BASED energy.

33번과의 차이:
  - PGM mode: discrete-V only (측정된 4,5,6,7,8V만 사용, 보간 X)
              실측 데이터로 가능한 동작만 시뮬
  - PGM energy: V × I_avg(during pulse) × t_pulse
                I_avg는 LTD curve의 시작/끝 평균 (측정 기반)
  - Inference energy 두 버전:
      v1 (lab/realistic): 광응답 시간 = 0.5s (측정상 한계)
      v2 (fast hypothetical): 광응답 시간 = 1 μs (NAND-like operation)
  - t_pulse default: 100 ms (lab measurement과 일치)

전체 파이프라인 (33번과 동일):
  Step 1: Device 측정 데이터 → PR + LTD model
  Step 2: DirectPDK 학습 → weight 분포
  Step 3: weight → I_target 매핑 (weight-aware bias)
  Step 4: discrete-V PGM 시뮬레이션 → I_actual
  Step 5: Inference (4-panel 비교)
  Step 6: Energy/Latency (CPU vs OKA, 두 버전 inference)

실행:
  python 34_oka_discrete_realdata.py
  python 34_oka_discrete_realdata.py --N-budget 3
  python 34_oka_discrete_realdata.py --t-pulse-ms 100  --inference-mode v1
"""
...
class LTDModel:
    ...
    def I_avg_during_pulse(self, V, user_n):
        """
        Cell이 user_n번째 펄스를 받는 동안의 평균 전류.
        측정 기반 PGM energy 계산용.

        펄스 전: I(user_n - 1)
        펄스 후: I(user_n)
        평균: (I_before + I_after) / 2

        Args:
          V: voltage level (must be in self.V_levels)
          user_n: 펄스 번호 (1-indexed)
        ...
        if user_n < 1:
            return 0.0
        # User_N=0 (펄스 가하기 전)은 fit n=1 (offset)
        # User_N=k (펄스 k번 가한 후) 는 fit n=k+1
        fit_n_before = user_n        # user N=k-1+1=k 번째 → before pulse k
        fit_n_after  = user_n + 1    # 펄스 k 가한 후
        I_before = float(self._double_exp(fit_n_before, *self.params[V]))
        I_after  = float(self._double_exp(fit_n_after,  *self.params[V]))
        return 0.5 * (I_before + I_after)
...
def direct_to_oka_mapping(weights, pr_model, ltd_model,
                          N_budget=4, V_max=8.0, I_max_native=None):
    """
...
def simulate_programming(I_target_map, ltd_model, N_budget=1, **kwargs):
    """
    각 cell의 target I_state를 실제 PGM 결과로 변환.

    측정된 V level (4, 5, 6, 7, 8V) × N ∈ [0, N_budget]만 사용.
    Continuous V 보간 없음 (실측 데이터에 충실).

    Returns:
      I_actual:  (H, W, 9)  실제 도달 I_state
      V_map:     (H, W, 9)  사용된 V (discrete 측정값 중 하나)
      N_map:     (H, W, 9)  사용된 펄스 수
      err_map:   (H, W, 9)  |achieved - target|
      Iavg_map:  (H, W, 9)  펄스 동안 평균 전류 [nA]
                            (현재 capacitive 모델로 PGM energy에 미사용,
                             reference로 유지)
    """
    ...
    Iavg_map = np.zeros_like(I_target_map, dtype=np.float64)

    has_iavg = hasattr(ltd_model, 'I_avg_during_pulse')
    ...
    for ix in range(H):
        for iy in range(W):
            for k in range(K):
                ...
                err_map[ix, iy, k]  = err
                if N > 0 and has_iavg:
                    I_avgs = [ltd_model.I_avg_during_pulse(V, n)
                              for n in range(1, N+1)]
                    Iavg_map[ix, iy, k] = float(np.mean(I_avgs))

    n_total = H * W * K
    n_zero  = int((N_map == 0).sum())
    print(f"\n[Programming] mode=discrete (measured V levels only)")
    print(f"   simulated {n_total} cells")
    print(f"   pulses/cell : avg={N_map.mean():.2f}, "
          f"max={int(N_map.max())}, no-pulse={n_zero} ({100*n_zero/n_total:.1f}%)")
    V_active = V_map[N_map > 0]
    if len(V_active) > 0:
        # V level 별 cell 분포
        for V_lvl in sorted(set(V_active.flatten().tolist())):
            n_v = (V_map == V_lvl).sum()
            print(f"   V={V_lvl:.1f}V used : {n_v} cells "
    ...
    Iavg_active = Iavg_map[N_map > 0]
    if len(Iavg_active) > 0:
        print(f"   I_avg/pulse: min={Iavg_active.min():.2f}, "
              f"max={Iavg_active.max():.2f}, "
              f"mean={Iavg_active.mean():.2f} nA")
    return I_actual, V_map, N_map, err_map, Iavg_map
...
# ╔══════════════════════════════════════════════════════════════════╗
# ║                                                                  ║
# ║   SECTION 7.  Energy / Latency models (measurement-based)        ║
# ║                                                                  ║
# ║   PGM energy: ½·C_gate·V² (capacitive charging)                  ║
# ║              gate leakage <1nA (측정 한계 미만) → 무시            ║
# ║              C_gate = 345 fF (Al2O3 29nm + Nb2O5 28nm series,    ║
# ║                                5×30 μm² active area)             ║
# ║                                                                  ║
# ║   Inference energy: E = V_read × I_photo × t_inference           ║
# ║                  I_photo는 PR 측정에서 직접                      ║
# ║                  t_inference 두 버전:                            ║
# ║                    v1 (realistic): 0.5s (lab 광응답 한계)        ║
# ║                    v2 (fast hyp.): 1 μs (NAND-like)              ║
# ║                                                                  ║
...
def oka_energy_latency(H, W, V_map, N_map, Iavg_map,
                        I_actual_map, P_image_uW, pr_model, info,
                        N_frames=1,
                        C_gate_fF=345.46,
                        V_read=1.0,
                        t_inference_us=1.0,
                        n_bit=8):
    """
    OKA 한 프레임 처리 energy/latency. 측정 데이터 기반.

    Programming은 1회로 보고 N_frames에 amortize.

    [PGM energy — capacitive model]
      이 device는 게이트 전류 < 1 nA (측정 한계 미만).
      → V × I_g × t_pulse 모델 부적절
      → Capacitive charging이 dominant:
        E_per_pulse_per_cell = ½ · C_gate · V²

      C_gate (이 device geometry):
        Active area: 5 × 30 = 150 μm²
        Stack: Al₂O₃ (29 nm, k=9) ‖ Nb₂O₅ (28 nm, k=45) 직렬
        → C_gate ≈ 345 fF/cell
      참고: Iavg_map 인자는 deprecated (하위 호환을 위해 유지)

    [Inference energy — measurement-based]
      각 cell당 한 번 read:
        I_photo = R(I_actual) · P_local        (PR 측정 기반)
        E_read  = V_read × I_photo × t_inference
      Σ over all cells (B, C, H, W, 9 taps)

    Args:
      V_map, N_map:   simulate_programming 출력
      Iavg_map:       (deprecated) 하위 호환만, 사용 X
      I_actual_map:   (H, W, 9) PGM 후 실제 I_state [nA]
      P_image_uW:     (H, W) 입력 이미지의 light power [μW/cm²]
      pr_model:       PhotoresponsivityModel
      info:           mapping info dict
      C_gate_fF:      cell capacitance [fF]
      V_read:         inference 시 cell read voltage [V]
      t_inference_us: per-frame photocurrent integration time [μs]
                       v1: 5e5 (=0.5s, lab 광응답 한계)
                       v2: 1   (=1μs, NAND-like fast hypothetical)

    Returns:
      e_amortized [pJ], total_ns [ns], breakdown dict
    """
    # ─── PGM energy (1회) — capacitive ½CV² ───────────────────
    ...
    # ─── Inference energy (per-frame) ─────────────────────────
    # 각 cell이 받는 빛은 입력 이미지의 9 neighbor patches로부터
    # I_photo = R(I_actual) · P_local
    # P_image_uW: (H, W),  3×3 unfold → (H, W, 9)
    H_, W_ = P_image_uW.shape
    P_pad = np.pad(P_image_uW, 1, mode='edge')
    P_patches = np.zeros((H_, W_, 9))
    for k, (dy, dx) in enumerate([(-1,-1),(-1,0),(-1,1),
                                    ( 0,-1),( 0,0),( 0,1),
                                    ( 1,-1),( 1,0),( 1,1)]):
        P_patches[:, :, k] = P_pad[1+dy:1+dy+H_, 1+dx:1+dx+W_]
    R_actual = pr_model.R(I_actual_map)              # (H, W, 9) [nA / μW/cm²]
    I_photo  = R_actual * P_patches                   # (H, W, 9) [nA]
    # E_read = V_read × I_photo × t_inference
    # V × nA × μs = V × 1e-9 A × 1e-6 s = 1e-15 J = fJ = 0.001 pJ
    e_inference = float((V_read * I_photo * t_inference_us * 1e-3).sum())  # [pJ]

    # 추가 inference 비용 (ADC, SRAM write — 한 column 한 번씩)
    n_pix = H * W
    e_adc    = W * ENERGY['adc_pj_sample'] * H        # row-sequential
    e_tia    = W * ENERGY['tia_pj_sample']
    e_sram_w = n_pix * n_bit * ENERGY['sram_read_pj_bit']
    e_periph = e_adc + e_tia + e_sram_w
    e_inference_total = e_inference + e_periph

    # ─── Amortized total ──────────────────────────────────────
    e_amortized = e_inference_total + e_pgm_total / N_frames

    # Latency: t_inference dominates (광응답 시간)
    # + ADC, SRAM write 추가
    t_total_ns = (t_inference_us * 1000.0
                  + H * LATENCY['adc_ns']
                  + LATENCY['sram_read_ns'])

    return e_amortized, t_total_ns, dict(
        pgm_total=e_pgm_total,
        pgm_amortized=e_pgm_total/N_frames,
        infer_photo=e_inference,
        infer_adc=e_adc, infer_tia=e_tia, infer_sram=e_sram_w,
        infer_total=e_inference_total,
        pulse_avg=pulse_avg, pulse_max=pulse_max,
        n_zero=n_zero,
        t_inference_us=t_inference_us,
        C_gate_fF=C_gate_fF)
...
def parse_args():
    ...
    p.add_argument("--C-gate-fF",  type=float, default=345.46,
                   help="Cell gate capacitance [fF]. "
                        ...
                        "Gate leakage <1nA, capacitive ½CV² dominant.")
    # Inference energy 파라미터
    p.add_argument("--V-read",     type=float, default=1.0,
                   help="Inference 시 cell read voltage [V]")
    p.add_argument("--inference-mode", default='both',
                   choices=['v1', 'v2', 'both'],
                   help="v1: t_inference=0.5s (lab 광응답 한계). "
                        "v2: t_inference=1μs (NAND-like fast hypothetical). "
                        "both: 두 버전 모두 출력 (default).")
    p.add_argument("--t-inference-v1-s",  type=float, default=0.5,
                   help="v1 inference time [s] (lab realistic)")
    p.add_argument("--t-inference-v2-us", type=float, default=1.0,
                   help="v2 inference time [μs] (fast hypothetical)")
    # 출력
    p.add_argument("--n-frames",   type=int, default=1,
                   help="programming amortization 기준 프레임 수")
    p.add_argument("--res-dir",    default="./res/oka_discrete_realdata")
    return p.parse_args()
...
def main():
    ...
    # PGM 시뮬레이션 → 실제 도달 I_state + I_avg trajectory
    I_actual_np, V_map, N_map, err_map, Iavg_map = simulate_programming(
    ...
    print(f"    saved: pdk_oka.pt (target + actual + pgm log + Iavg)")
    ...
    # ┌─────────────────────────────────────────────────────────────┐
    # │  Step 5. Energy / Latency (CPU vs OKA v1, v2)                │
    # │    OKA 두 버전:                                              │
    # │      v1 (realistic): t_inference = 0.5 s (lab 광응답 한계)   │
    # │      v2 (fast hyp.): t_inference = 1 μs (NAND-like)         │
    # └─────────────────────────────────────────────────────────────┘
    print("\n" + "="*60)
    print("[Step 5] Energy / Latency comparison (CPU vs OKA)")
    print("="*60)

    e_cpu, t_cpu, br_cpu = cpu_energy_latency(H, W)

    # Inference에 쓸 입력 이미지 light power 분포 (degraded image 사용)
    P_image_uW = (deg.squeeze().numpy() * args.P_scale)   # [μW/cm²]

    # OKA v1: realistic lab 광응답 시간
    e_v1, t_v1, br_v1 = oka_energy_latency(
        H, W, V_map, N_map, Iavg_map,
        I_actual_np, P_image_uW, pr_model, info,
        N_frames=args.n_frames,
        C_gate_fF=args.C_gate_fF,
        V_read=args.V_read,
        t_inference_us=args.t_inference_v1_s * 1e6)   # 0.5s → 5e5 μs
    # OKA v2: fast hypothetical
    e_v2, t_v2, br_v2 = oka_energy_latency(
        H, W, V_map, N_map, Iavg_map,
        I_actual_np, P_image_uW, pr_model, info,
        N_frames=args.n_frames,
        C_gate_fF=args.C_gate_fF,
        V_read=args.V_read,
        t_inference_us=args.t_inference_v2_us)
    ...
    def print_oka(label, e_amort, t_ns, br):
        print(f"\n  OKA-{label} per-frame "
              f"(amortized over {args.n_frames} frames):")
        print(f"    Total energy : {e_amort/1e6:>10.3f} μJ")
        print(f"    Latency      : {t_ns/1e6:>10.3f} ms")
        print(f"    PGM (1회)    : {br['pgm_total']/1e6:.2f} μJ "
              f"→ amortized {br['pgm_amortized']/1e6:.4f} μJ/frame")
        print(f"      pulses/cell: avg={br['pulse_avg']:.2f}, "
              f"max={br['pulse_max']}, "
              f"no-pulse={br['n_zero']} ({100*br['n_zero']/(H*W*9):.1f}%)")
        print(f"      energy model: ½·C_gate·V², "
              f"C_gate={br['C_gate_fF']:.1f} fF (capacitive)")
        print(f"    Inference:")
        print(f"      photo (cell): {br['infer_photo']/1e6:.4f} μJ  "
              f"(t_inf={br['t_inference_us']:.1f} μs)")
        print(f"      ADC         : {br['infer_adc']/1e6:.4f} μJ")
        print(f"      TIA         : {br['infer_tia']/1e6:.4f} μJ")
        print(f"      SRAM write  : {br['infer_sram']/1e6:.4f} μJ")
        print(f"      total infer : {br['infer_total']/1e6:.4f} μJ/frame")

    if args.inference_mode in ('v1', 'both'):
        print_oka(f"v1 (lab, t_inf={args.t_inference_v1_s}s)",
                  e_v1, t_v1, br_v1)
    if args.inference_mode in ('v2', 'both'):
        print_oka(f"v2 (fast, t_inf={args.t_inference_v2_us}μs)",
                  e_v2, t_v2, br_v2)

    print(f"\n  Comparison (CPU / OKA):")
    if args.inference_mode in ('v1', 'both'):
        print(f"    v1 (lab) : energy={e_cpu/e_v1:>8.2f}×, "
              f"latency={t_cpu/t_v1:>8.2f}×")
    if args.inference_mode in ('v2', 'both'):
        print(f"    v2 (fast): energy={e_cpu/e_v2:>8.2f}×, "
              f"latency={t_cpu/t_v2:>8.2f}×")

    # CSV summary
    import csv
    with open(os.path.join(args.res_dir, '34_summary.csv'), 'w') as f:
        w = csv.writer(f)
        w.writerow(['metric', 'CPU', 'OKA-v1', 'OKA-v2',
                    'v1 ratio', 'v2 ratio'])
        w.writerow(['Energy_uJ',
                    f'{e_cpu/1e6:.3f}',
                    f'{e_v1/1e6:.3f}',
                    f'{e_v2/1e6:.3f}',
                    f'{e_cpu/e_v1:.2f}',
                    f'{e_cpu/e_v2:.2f}'])
        w.writerow(['Latency_us',
                    f'{t_cpu/1e3:.3f}',
                    f'{t_v1/1e3:.3f}',
                    f'{t_v2/1e3:.3f}',
                    f'{t_cpu/t_v1:.2f}',
        ...
        w.writerow(['Pulses_per_cell_avg', '-',
                    f'{br_v1["pulse_avg"]:.2f}', '-', '-', '-'])
    print(f"\n[Saved] {os.path.join(args.res_dir, '34_summary.csv')}")
