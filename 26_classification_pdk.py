...
def load_mnist(n_train, n_test, img_size=28):
    try:
        import torchvision
        ...
        test_ds  = torchvision.datasets.MNIST(
            root='/tmp/mnist', train=False, download=True, transform=tf)
        ...
        print(f"  [MNIST] train={len(X_tr)}, test={len(X_te)}, size={img_size}×{img_size}")
...
def save_epoch_curves(history_fc, history_cnn, n_classes, res_dir):
    ...
    fig.suptitle(
        f'Classification accuracy per epoch  ({n_classes}-class MNIST)\n'
...
def parse_args():
    ...
    p.add_argument('--res-dir',    type=str,   default='./res/classification')
    return p.parse_args()
...
def main():
    ...
    # ── 데이터 로드 ──────────────────────────────────────────
    print('[Load] MNIST...')
    X_tr, y_tr, X_te, y_te, n_classes = load_mnist(
