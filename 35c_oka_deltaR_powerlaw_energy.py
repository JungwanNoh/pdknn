...
"""
35c_oka_deltaR_powerlaw_energy.py
======================================================================
...
def make_comparison_scheme_table(args, out_csv, out_png):
    rows = [
        ...
        ("Programming step", "-", "Discrete PGM states from measured LTD data"),
        ("One-time PGM energy", "-", "Σ 1/2 C_gate V_PGM^2 N_pulse"),
        ...
        ("Output write", "Write processed digital image", "Write ADC output only"),
        ...
        ("Lab scenario", "-", f"read pulse = {args.lab_read_ms} ms"),
        ("Fast scenario", "-", f"read pulse = {args.fast_read_us} µs"),
        ("Main advantage", "-", "Reduced digital MAC and PDK data movement"),
...
def parse_args():
    ...
    p.add_argument("--res-dir", default="./res/oka35c_deltaR_powerlaw")
    return p.parse_args()
...
def main():
    ...
    I_actual, V_map, N_map, err_map = simulate_discrete_programming(I_target, ltd_model, args.N_budget)
    ...
    print(f"  PGM total = {e_pgm_total_pJ/1e6:.6f} uJ, amortized = {e_pgm_amort_pJ/1e6:.6f} uJ/frame")
    print(f"  latency = {t_oka_lab_ns/1e6:.6f} ms")
    ...
    print(f"  PGM total = {e_pgm_total_pJ/1e6:.6f} uJ, amortized = {e_pgm_amort_pJ/1e6:.6f} uJ/frame")
    print(f"  latency = {t_oka_fast_ns/1e6:.6f} ms")
    ...
    summary_path = os.path.join(args.res_dir, "35c_summary.csv")
