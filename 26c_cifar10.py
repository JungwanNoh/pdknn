...
실험 구성:
  Dataset: CIFAR-10 (10-class, 32×32, grayscale)
...
def degrade_coma_vig(x, sigma0=0.20, alpha_psf=1.5,
    ...
    """
    MNIST 28×28 grayscale 기준 degradation.
...
def load_cifar10(n_train, n_test, img_size=28):
    try:
        import torchvision
        import torchvision.transforms as T
        tf = T.Compose([
            T.Resize(img_size),
            T.Grayscale(1),        # CIFAR-10은 RGB → grayscale 변환
            T.ToTensor(),
        ])
        train_ds = torchvision.datasets.CIFAR10(
            root='/tmp/cifar10', train=True,  download=True, transform=tf)
        test_ds  = torchvision.datasets.CIFAR10(
            root='/tmp/cifar10', train=False, download=True, transform=tf)
        ...
        print(f"  [CIFAR-10] train={len(X_tr)}, test={len(X_te)}, size={img_size}×{img_size}")
...
def _synthetic_data(n_train, n_test, img_size=28):
    """MNIST 없을 때: 숫자 모양 synthetic 10-class"""
    n_classes = 10
    def make(n):
        X, y = [], []
        for i in range(n):
...
def save_epoch_curves(history_fc, history_cnn, n_classes, res_dir):
    ...
    fig.suptitle(
        f'Classification accuracy per epoch  ({n_classes}-class CIFAR-10 grayscale)\n'
...
def parse_args():
    ...
    p.add_argument('--res-dir',    type=str,   default='./res/classification_cifar10')
    return p.parse_args()
...
def main():
    ...
    # ── 데이터 로드 ──────────────────────────────────────────
    print('[Load] CIFAR-10...')
    X_tr, y_tr, X_te, y_te, n_classes = load_cifar10(
