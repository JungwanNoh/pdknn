"""
14_non_uniform_illum_cifar10.py
============================================
Non-uniform Illumination -- 3x3 PD Kernel [CIFAR-10]
Run: python 14_non_uniform_illum_cifar10.py
"""
...
def load_dataset(args):
    try:
        ...
        ds=torchvision.datasets.CIFAR10(root=args.data_dir,train=False,download=True,transform=tf)
        imgs,idx=[],list(range(0,len(ds),max(1,len(ds)//args.n_images)))
        for i in idx[:args.n_images]:
            img,_=ds[i]; imgs.append(img.unsqueeze(0))
        print(f'[Load] CIFAR-10: {len(imgs)} images'); return imgs
...
def _syn(n,size):
    ...
    pts=[(0.5+0.5*torch.sign(torch.sin(gy*8)*torch.sin(gx*8))).clamp(0,1),
         (0.5+0.25*torch.sin(gy*8)+0.25*torch.sin(gx*8)).clamp(0,1),
         ((gx+1)/2).clamp(0,1),
         (0.5+0.5*torch.cos((gy**2+gx**2).sqrt()*10)).clamp(0,1)]
    return [p.unsqueeze(0).unsqueeze(0) for p in pts[:n]]

def parse_args():
    p=argparse.ArgumentParser(description='Non-uniform Illum PDK -- CIFAR-10')
    ...
    p.add_argument('--res-dir',       type=str,   default='./res/nonunif_cifar10')
    return p.parse_args()
