"""
10_vignetting_div2k.py
========================================
3x3 Position-Dependent Kernel Vignetting correction — DIV2K
...
Run:
  python 10_vignetting_div2k.py
  python 10_vignetting_div2k.py --n-images 4 --alpha 2.5
  python 10_vignetting_div2k.py --data-dir /path/to/DIV2K
"""
import argparse
import os
import glob
...
# ============================================================================
# Dataset: DIV2K (DIVerse 2K resolution)
# Super resolution 분야 표준. 고해상도라 blur 효과가 잘 보임.
# 다운로드: https://data.vision.ee.ethz.ch/cvl/DIV2K/
# 또는:     pip install basicsr  (자동 다운로드 가능)
# 경로 예시: --data-dir ./data/DIV2K/DIV2K_valid_HR
# ============================================================================
...
def load_dataset(args):
    ...
    if args.data_dir and os.path.isdir(args.data_dir):
        ...
        if imgs:
            print(f'[Load] DIV2K: {len(imgs)} images from {args.data_dir}')
            return imgs

    # 2) basicsr 자동 다운로드 시도
    try:
        from basicsr.utils import scandir
        print('[Info] basicsr found but data-dir not specified.')
        print('[Info] Please run with --data-dir /path/to/DIV2K_valid_HR')
    except ImportError:
        pass

    # 3) 고해상도 합성 이미지 fallback
    print('[Fallback] DIV2K not found — using high-res synthetic images')
    imgs = []
    H = W = args.img_size
    ...
    for p in patterns[:args.n_images]:
        imgs.append(p.unsqueeze(0).unsqueeze(0))
    print(f'[Fallback] Generated {len(imgs)} synthetic patterns at {H}x{W}')
    return imgs


def parse_args():
    p = argparse.ArgumentParser(description='Vignetting PDK — DIV2K')
    p.add_argument('--data-dir',  type=str,   default='',
                   help='DIV2K HR 이미지 폴더 (비워두면 합성 이미지 사용)')
    p.add_argument('--n-images',  type=int,   default=4)
    p.add_argument('--img-size',  type=int,   default=512,
                   help='crop size (DIV2K는 2K 원본, 여기서 crop)')
    p.add_argument('--alpha',     type=float, default=2.5)
    p.add_argument('--n-iter',    type=int,   default=300)
    p.add_argument('--lr',        type=float, default=0.05)
    p.add_argument('--res-dir',   type=str,   default='./res/vignetting_div2k')
    return p.parse_args()
...
def main():
    ...
    save_comparison(raw_list, deg_list, cor_ana_list, cor_learn_list,
                    true_alpha, est_alpha, learn_alpha, args.res_dir, 'DIV2K')
