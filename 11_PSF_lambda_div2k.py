...
11_PSF_div2k.py
============================================
Position-dependent PSF blur only -- 3x3 PD Kernel [DIV2K]
...
Run:
  python 11_PSF_div2k.py
  python 11_PSF_div2k.py --sigma0 0.3 --alpha-psf 1.2 --n-iter 500
"""
...
# ============================================================================
# Restoration: 3x3 PD Kernel (lambda separated)
# ============================================================================

class PSFRestorationPDK(nn.Module):
    """
    Lambda 분리 버전 — 파라미터 4개.

    기존 문제:
      lambda(r) = sigma(r) * 1.5  로 고정
      → sigma 과추정으로 edge deblur 과보정

    개선:
      sigma(r)  = sigma0  + alpha_psf * r²  → PSF blur 모델링 전용
      lambda(r) = lambda0 + alpha_lam * r²  → deblur 강도 전용 (독립)

    효과:
      sigma는 degradation PSF를 정확히 모델링
      lambda는 복원 강도를 독립적으로 최적화
      → alpha_psf 과추정 없이 각자 최적값으로 수렴
    """
    def __init__(self, H, W,
        ...
        # Deblur 강도 파라미터 (독립)
        self._lambda0   = nn.Parameter(torch.tensor(
            float(np.log(np.exp(lambda0_init)-1.0))))
        self._alpha_lam = nn.Parameter(torch.tensor(
            float(np.log(np.exp(alpha_lam_init)-1.0))))
...
def save_comparison(raw_list, deg_list, cor_list,
    ...
    cols = [
        ...
        (cor_list, '3. Corrected (learned PDK)',
         ...
         f'  a_lam={learned_params["alpha_lam"]:.2f}'),
    ]
...
def save_kernel_map(true_params, learned_params, loss_history, size, res_dir):
    ...
    def _kres(s0,ap,l0,al,rv):
        co=np.array([[-1,-1],[-1,0],[-1,1],[0,-1],[0,0],[0,1],[1,-1],[1,0],[1,1]],dtype=float)
        d2=(co**2).sum(-1); s=max(s0+ap*rv**2,0.1)
        g=np.exp(-d2/(2*s**2)); g=g/g.sum()
        delta=np.zeros(9); delta[4]=1.0
        lam=min(max(l0+al*rv**2, 0.05), 3.0)   # lambda 분리
    ...
    for row,(zn,zr) in enumerate(zones):
        ...
        _draw_3x3(ax,_kres(s0t,apt,s0t*1.5,0.0,zr),f'Restore (true params)\nr={zr:.2f}')
        ax=fig.add_subplot(gs[row+1,2])
        _draw_3x3(ax,_kres(s0l,apl,learned_params.get('lambda0',0.3),learned_params.get('alpha_lam',0.5),zr),f'Restore (learned)\nr={zr:.2f}')
        ax=fig.add_subplot(gs[row+1,3])
        kd=_kdeg(s0t,apt,zr); kr=_kres(s0l,apl,learned_params.get('lambda0',0.3),learned_params.get('alpha_lam',0.5),zr)
...
def save_sigma_profile(true_params, learned_params, res_dir):
    ...
    # true: lambda tied to sigma (old behavior, reference)
    l0t = s0t * 1.5; alt = 0.0
    l0l = learned_params.get('lambda0', 0.3)
    all_ = learned_params.get('alpha_lam', 0.5)
    axes[1].plot(r, np.minimum(l0t + alt*r2, 3.0),
                 label=f'lambda (true ref)  l0={l0t:.2f} a=0.00',
                 color='black', linewidth=2)
    axes[1].plot(r, np.minimum(l0l + all_*r2, 3.0),
                 label=f'lambda (learned)  l0={l0l:.3f} a={all_:.3f}',
                 color='tomato', linewidth=1.5, linestyle='--')
    axes[1].set_xlabel('r'); axes[1].set_ylabel('lambda(r)')
    axes[1].set_title('Deblur strength (lambda separated)')
...
def load_dataset(args):
    if args.data_dir and os.path.isdir(args.data_dir):
        ...
        if imgs: print(f'[Load] DIV2K: {len(imgs)} images'); return imgs
    print('[Fallback] DIV2K not found -- synthetic patterns')
    return _synthetic(args.n_images, args.img_size)

def _synthetic(n, size):
    H=W=size; ys=torch.linspace(-1,1,H); xs=torch.linspace(-1,1,W)
    gy,gx=torch.meshgrid(ys,xs,indexing='ij')
    pts=[(0.5+0.25*torch.sin(gy*25)+0.25*torch.sin(gx*25)).clamp(0,1),
         (0.5+0.5*torch.cos((gy**2+gx**2).sqrt()*18)).clamp(0,1),
         ((gy+gx+1)/2).clamp(0,1),
         (0.5+0.5*torch.sign(torch.sin(gy*15)*torch.sin(gx*15))).clamp(0,1)]
    return [p.unsqueeze(0).unsqueeze(0) for p in pts[:n]]

def parse_args():
    p=argparse.ArgumentParser(description='PSF only PDK -- DIV2K')
    ...
    p.add_argument('--res-dir',   type=str,   default='./res/psf_only_div2k')
    return p.parse_args()
...
def main():
    ...
    print(f'[Learned img1] sigma0={learned_params["sigma0"]:.3f}  alpha_psf={learned_params["alpha_psf"]:.3f}'
          f'  lambda0={learned_params["lambda0"]:.3f}  alpha_lam={learned_params["alpha_lam"]:.3f}')
    print('[All imgs] ' + '  '.join(
