...
"""
35h_oka_deltaR_powerlaw_continuous_ispp.py
======================================================================
CPU DirectPDK vs OKA DeltaR-PDK simulation with power-law photoresponse,
discrete programming, CDS/read-pulse energy, energy/latency plots, and
...
After training:
    DeltaR -> R_target = R_ref + DeltaR
             ...
             -> I_actual through measured discrete LTD programming states
...
def simulate_discrete_programming(I_target, ltd_model, N_budget):
    H, W, K = I_target.shape
    I_actual = np.zeros_like(I_target, dtype=float)
    V_map = np.zeros_like(I_target, dtype=float)
    N_map = np.zeros_like(I_target, dtype=np.int32)
    err_map = np.zeros_like(I_target, dtype=float)

    for y in range(H):
        for x in range(W):
            for k in range(K):
                V, N, achieved, err = ltd_model.best_V_N(float(I_target[y, x, k]), N_max=N_budget)
                I_actual[y, x, k] = achieved
                V_map[y, x, k] = V
                N_map[y, x, k] = N
                err_map[y, x, k] = err
...
def estimate_continuous_ispp_programming(I_target, ltd_model, N_budget=0, mode="target_exact"):
    ...
    Purpose:
      Avoid the overly conservative assumption that a user can only choose a
      small number of fixed pulse counts. In practical memory operation, the
      state can be adjusted through program-and-verify / ISPP until the target
      current or responsivity is reached.

    mode:
      target_exact:
        I_actual = I_target, i.e., verify reaches target.
        Programming log is only used for energy estimation.

    Energy-log estimation:
      For each target I, find the nearest measured (V,N) combination in the
      full LTD measurement range, unless N_budget > 0. This gives a traceable
      capacitive PGM energy estimate while the achieved state is still treated
      as verified to target.

      If target is close to the initial state, no pulse is assigned.

    Returns:
      I_actual, V_map, N_map, err_map
    """
    if mode != "target_exact":
        raise ValueError("Only target_exact is supported for continuous ISPP mode.")

    H, W, K = I_target.shape
    I_actual = I_target.copy().astype(float)
    V_map = np.zeros_like(I_target, dtype=float)
    N_map = np.zeros_like(I_target, dtype=np.int32)
    err_map = np.zeros_like(I_target, dtype=float)

    # Use full measured pulse range by default.
    N_eff = int(ltd_model.N_meas_max) if (N_budget is None or int(N_budget) <= 0) else int(N_budget)

    for y in range(H):
        for x in range(W):
            for k in range(K):
                V, N, achieved, _ = ltd_model.best_V_N(float(I_target[y, x, k]), N_max=N_eff)
                V_map[y, x, k] = V
                N_map[y, x, k] = N
                # ISPP verify assumption: achieved state equals target.
                err_map[y, x, k] = 0.0

    n_total = H * W * K
    n_zero = int((N_map == 0).sum())
    print("[Continuous ISPP programming]")
    print("  mode: target_exact program-and-verify approximation")
    print(f"  programming energy log uses nearest measured V,N over 0 ~ {N_eff} pulses")
    print(f"  total cells = {n_total:,}")
    print(f"  avg estimated pulses/cell = {N_map.mean():.6f}, max = {int(N_map.max())}")
    print(f"  estimated no-pulse cells = {n_zero:,} ({100*n_zero/n_total:.3f}%)")
    print(f"  PGM state error after verify = avg {err_map.mean():.6f} nA, max {err_map.max():.6f} nA")
    print(f"  I_actual range = {I_actual.min():.6f} ~ {I_actual.max():.6f} nA")
...
def make_comparison_scheme_table(args, out_csv, out_png):
    rows = [
        ...
        ("Main limitation", "-", "Depends on programming accuracy and readout speed"),
    ]
...
def parse_args():
    ...
    p.add_argument("--res-dir", default="./res/oka35h_deltaR_powerlaw_continuous_ispp")
    return p.parse_args()
...
def main():
    ...
    print("\n" + "=" * 72)
    print("[3] Map trained DeltaR to discrete programmed states")
    print("=" * 72)
    ...
    summary_path = os.path.join(args.res_dir, "35h_summary.csv")
