...
class KANHyperPDKConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.bmm(
            patches.reshape(batch * self.H_out * self.W_out, 1, -1),
            kernels.unsqueeze(0).expand(batch, -1, -1, -1).reshape(
                batch * self.H_out * self.W_out, -1, self.out_channels
            )
        ).squeeze(1)

        out = out.view(batch, self.H_out * self.W_out, self.out_channels)
        out = out + biases.unsqueeze(0)
        out = out.permute(0, 2, 1).view(batch, self.out_channels, self.H_out, self.W_out)
        return out
...
def _apply_kernel_rgb(x: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    channels = x.shape[1]
    weight = kernel.to(x.device, x.dtype).view(1, 1, 3, 3).repeat(channels, 1, 1, 1)
    y = F.conv2d(x, weight, padding=1, groups=channels)
    return torch.clamp(y, -3.0, 3.0)


def global_preprocess_batch(x: torch.Tensor) -> torch.Tensor:
    global_k = _make_kernel('binomial')
    return _apply_kernel_rgb(x, global_k)
...
def get_cifar10_loaders(batch_size: int = 128, num_workers: int = 2, data_root: str = './data'):
    ...
    train_set = torchvision.datasets.CIFAR10(data_root, train=True, download=True, transform=transform_train)
    test_set = torchvision.datasets.CIFAR10(data_root, train=False, download=True, transform=transform_test)
...
def train_one_epoch(model, loader, optimizer, criterion, device, preprocess: str):
    ...
    for imgs, labels in loader:
        ...
        if preprocess == 'global':
            ...
            for key in zone_accum:
                zone_accum[key] += zone_stats[key]
            zone_batches += 1
...
def main():
    parser = argparse.ArgumentParser(description='Global / PDK preprocessed CIFAR-10 classification with HyperPDK model')
    parser.add_argument('--model', type=str, default='KANHyperPDKNN',
                        choices=['KANHyperPDKNN', 'BaselineCNN'])
    parser.add_argument('--preprocess', type=str, default='global',
    ...
    for name, model in models.items():
        print(f'{name:20s} | params: {count_params(model):>10,}')
    ...
    for epoch in range(1, args.epochs + 1):
        tr_loss, tr_acc, train_zone = train_one_epoch(
            model, train_loader, optimizer, criterion, device, args.preprocess
        )
        te_loss, te_acc, _ = evaluate(
            model, test_loader, criterion, device, args.preprocess
        )
        ...
        msg = (
            ...
            f'best={best_acc:.3f} | lr={current_lr:.6e}'
        )
        if lr_changed:
